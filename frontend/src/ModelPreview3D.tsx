...
function WingHalf({ side, halfSpan, rootChord, tipChord, sweep, dihedral }: {
  side: -1 | 1; halfSpan: number; rootChord: number; tipChord: number; sweep: number; dihedral: number;
}) {
  const geometry = useMemo(() => {
    const shape = new THREE.Shape();
    shape.moveTo(0, 0);
    shape.lineTo(side * halfSpan, sweep);
    shape.lineTo(side * halfSpan, sweep + tipChord);
    ...
    return result;
  }, [halfSpan, rootChord, side, sweep, tipChord]);
...
function Aircraft({ evaluation }: { evaluation: PluginEvaluation }) {
  ...
  const tipChord = evaluation.parameters.wing.tipChordMm;
  const sweep = Math.max(...xs) - tipChord;
...
export function ModelPreview3D({ evaluation, view, autoRotate }: {
  ...
  return <div className="model-preview-3d" role="img" aria-label="Interaktive dreidimensionale Vorschau des Flugmodells">
    <Canvas dpr={[1, 1.75]} shadows gl={{ antialias: true, powerPreference: "high-performance" }}>
      ...
      <PerspectiveCamera key={view} makeDefault position={cameraPosition} fov={38} />
